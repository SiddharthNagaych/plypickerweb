...
export default function ServiceFormComponent({ city, onSaved }: ServiceFormProps) {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white p-6 border rounded shadow">
      <h2 className="text-2xl font-semibold text-orange-600 mb-4">🛎️ Service Upload</h2>
      ...
      <div>
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="border p-4 mb-4 rounded space-y-2">
            ...
            <textarea {...register(`vars.${index}.serviceDescription`)} placeholder="Variant Description" className="w-full border px-3 py-2 rounded" rows={2} />
            <input {...register(`vars.${index}.attrs.Variants`)} placeholder="Attributes" className="w-full border px-3 py-2 rounded" />
            <textarea
            ...
            <textarea
              placeholder='Discounted Prices JSON [{"city":"Pune","price":123}]'
              onBlur={(e) => {
                try {
                  const parsed = JSON.parse(e.target.value);
                  setValue(`vars.${index}.discounted_price`, parsed);
                } catch {
                  alert("❌ Invalid JSON for discounted_price.");
                }
              }}
              className="w-full border px-3 py-2 rounded"
            />
            <input
              ...
              className="w-full border px-3 py-2 rounded"
            />
            <button type="button" onClick={() => remove(index)} className="text-red-600 underline">
              ❌ Remove Variant
            </button>
          </div>
        ))}
